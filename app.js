...
async function guardarGasto(id) {
  ...
  const idNum = id && id !== 'null' ? Number(id) : null;
  if(idNum) {
    await sb.from('gastos').update(data).eq('id', idNum);
  } else {
  ...
  await cargarDatos();
  renderVista();
  toast(idNum ? 'Gasto actualizado' : (esRecurrente && mesesRec > 1 ? `Gasto creado · ${mesesRec} meses programados` : 'Gasto creado'));
}
...
async function guardarIngreso(id) {
  ...
  const idReal = (id && id !== 'null' && id !== null) ? Number(id) : null;
  let err;
  if(idReal) {
    const res = await sb.from('ingresos').update(data).eq('id', idReal);
    err = res.error;
  } else {
    const res = await sb.from('ingresos').insert(data);
    err = res.error;
  }
  if(err) { console.error('Error ingreso:', err); toast('Error: ' + err.message); return; }
  cerrarModal();
  await cargarDatos();
  cambiarMes(mesFecha);
  toast(idReal ? 'Ingreso actualizado' : 'Ingreso creado');
}
